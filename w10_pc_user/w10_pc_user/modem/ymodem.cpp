...
extern HWND g_hdlg;
extern void doWriteMsg(TCHAR* msg);


BOOL BCB2WIN::Init(TCHAR* buf) {
	updateio = new ComIO(buf);
	return updateio->InitComIO(TRUE);
}

void BCB2WIN::UART_CLOSE(void) {
	delete updateio;
	updateio = NULL;
}

int BCB2WIN::UART_READ(char *dataptr,DWORD bytes_to_read,DWORD *len) {
	int result = 0;
	if(updateio) {
		*len = updateio->ReadCom(dataptr, bytes_to_read);
		result = 1;
	}
	return result;
}

int BCB2WIN::UART_WRITE(char *dataptr,DWORD bytes_to_write,DWORD *len) {
	int result = 0;
	if(updateio) {
		*len = updateio->WriteCom(dataptr, bytes_to_write);
		result = 1;
	}
	return result;
}

void BCB2WIN::ShowTestMessage(char *format, ...) {
	if( ! IsWindow(g_hdlg)) {
		return;
	}
	char				data[16384];
	...
	vsnprintf_s(data, 16384, _TRUNCATE,format,arg_pt);
	len = strlen(data);
	TCHAR bcbmsg[16384];
	swprintf(bcbmsg, 16384, L"%S", data);
	doWriteMsg(bcbmsg);
}


BCB2WIN * Form1 = new BCB2WIN();
...
static  int32_t Receive_Byte (uint8_t *c, uint32_t timeout)
{
  ...
  while (timeout-- > 0)
  {
      if(!Form1->UART_READ((char*)c,1,&len)){
         break;
      }
      if(len == 1){
         return 0;
...
static uint32_t Send_Byte (uint8_t c)
{
  DWORD len,i;
  for(i=0;i<25;i++){
      if(!Form1->UART_WRITE((char*)&c,1,&len)){
         break;
      }
      if(len == 1){
         break;
...
void Ymodem_SendPacket(uint8_t *data, uint16_t length)
{
  ...
  for(index=0;index<length;){
      ...
      if(!Form1->UART_WRITE((char*)(data+index),writelen,&writelen)){
         return;
      }
      index += (uint16_t)writelen;
  }
}
...
uint8_t Ymodem_Transmit (uint8_t *buf, const uint8_t* sendFileName, uint32_t sizeFile)
{
  ...
  {
    // Prepare next packet
    Ymodem_PreparePacket((uint8_t*)buf_ptr, &packet_data[0], (uint8_t)blkNumber, (uint32_t)size);
    // Send next packet
    ...
    SendDlgItemMessage(g_hdlg, IDC_DOWNLOAD, PBM_SETPOS, 100*(sizeFile - size)/sizeFile, 0);
  }
